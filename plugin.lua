...
function OnSetText(uri, text)
	...
	-- prevent diagnostic errors from <const> and <close> attributes
	for start, finish in str_gmatch(text, '()<%s*[Cc][Oo][Nn][Ss][Tt]%s*>()') do
		if not isDead(start) then
			count = count + 1
			diffs[count] = {
				start = start,
	...
	for start, finish in str_gmatch(text, '()<%s*[Cc][Ll][Oo][Ss][Ee]%s*>()') do
		if not isDead(start) then
			count = count + 1
			diffs[count] = {
				start = start,
